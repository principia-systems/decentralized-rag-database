...
class DatabaseCreator:
    ...
    def batch_retrieve_data(self, embedding_cids, content_cids):
        ...
        try:
            ...
            if result["failed_embeddings"]:
                self.logger.warning(
                    f"Failed to retrieve {len(result['failed_embeddings'])} embeddings: {result['failed_embeddings']}"
                )
            if result["failed_contents"]:
                self.logger.warning(
                    f"Failed to retrieve {len(result['failed_contents'])} contents: {result['failed_contents']}"
                )

            self.logger.info(
                f"Successfully batch retrieved {len(result['embeddings'])} embeddings and {len(result['contents'])} contents"
            )
            ...
            try:
                ...
                if metadata_cid in result.get("contents", {}):
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Retrieved metadata for PDF {pdf_cid}: {metadata.get('title', 'Unknown Title')}"
                        )
                        return metadata
                    except json.JSONDecodeError as e:
                        self.logger.error(
                            f"Failed to parse metadata JSON for PDF {pdf_cid}: {e}"
                        )
                        return None
                else:
                    self.logger.warning(
                        f"Metadata content not found for CID {metadata_cid}"
                    )
                    return None
    ...
    def process_paths(self, start_cid, path, db_name):
        ...
        if pdf_metadata:
            self.logger.info(
                f"Retrieved metadata: '{pdf_metadata.get('title', 'Unknown Title')}' by {pdf_metadata.get('authors', ['Unknown Authors'])}"
            )
        else:
            self.logger.warning(
                f"No metadata available for PDF {start_cid}, proceeding without metadata"
            )
        ...
        self.logger.info(
            f"Preparing batch retrieval for {len(embedding_cids)} embeddings and {len(content_cids)} contents"
        )

        # Batch retrieve all data from light server
        embeddings_dict, contents_dict = self.batch_retrieve_data(
            embedding_cids, content_cids
        )
        ...
        for i, path_info in enumerate(path_metadata):
            ...
            if embedding_cid not in embeddings_dict:
                self.logger.error(
                    f"Missing embedding for CID {embedding_cid}, skipping"
                )
                skipped_count += 1
                continue
        ...
        if batch_embeddings:
            try:
                ...
                self.logger.info(
                    f"Batch inserted {successful_inserts} documents into '{db_name}' with PDF metadata"
                )
            except Exception as e:
                self.logger.error(
                    f"Failed to batch insert documents into '{db_name}': {e}"
                )
        else:
        ...
        if skipped_count > 0:
            self.logger.warning(
                f"Skipped {skipped_count} documents due to missing data"
            )

        self.logger.info(
            f"Successfully inserted {successful_inserts}/{len(path_metadata)} documents into '{db_name}' with PDF metadata"
        )
