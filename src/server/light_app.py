    ...
    try:
        ...
        if email in whitelisted_emails:
            return AddEmailResponse(
                success=False,
                message=f"Email {email} is already whitelisted.",
                email=email,
            )
        ...
        return AddEmailResponse(
            success=True, message=f"Email {email} added to whitelist.", email=email
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error adding email to whitelist: {str(e)}"
        )


@app.post("/api/research/scrape")
async def scrape_research_papers(
    request: ResearchScrapeRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        config = ScraperConfig.from_research_area(
            research_area=request.research_area.strip(), user_email=request.user_email
        )
        ...
        (
            ...
            _thread_pool, _scrape_papers_sync, scraper, True  # cleanup_pdfs=True
        )

        if success and zip_path:
            user_logger.info(
                f"Successfully completed scraping for {request.user_email}"
            )
            ...
            safe_topic = "".join(
                c
                for c in request.research_area[:30]
            ...
            user_logger.error(
                f"Scraping failed for {request.user_email}: {result_message}"
            )
...
async def batch_retrieve_ipfs_data(request: BatchRetrievalRequest):
    ...
    try:
        ...
        user_logger.info(
            f"Starting batch retrieval for {len(request.embedding_cids)} embeddings and {len(request.content_cids)} contents"
        )
        ...
        user_logger.info(
            f"Successfully retrieved {len(embeddings)}/{len(request.embedding_cids)} embeddings and "
            f"{len(contents)}/{len(request.content_cids)} contents"
        )
...
async def get_evaluation_stats(user_email: str):
    ...
    try:
        ...
        stats = {"manual": 0, "scoring": 0, "ranking": 0, "total": 0}
            ...
            if file_path.exists():
                try:
                    with open(file_path, "r") as f:
                        ...
                        user_evaluations = [
                            e for e in evaluations if e.get("user_email") == user_email
                        ]
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting evaluation stats: {str(e)}"
        )
