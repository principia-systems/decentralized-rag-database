...
def process_batch_on_gpu(model_loader_func, batch_texts: List[str], device: torch.device, batch_idx: int) -> Dict:
    ...
    try:
        ...
        if torch.is_tensor(embeddings):
            ...
            embeddings_cpu = embeddings.tolist() if hasattr(embeddings, 'tolist') else list(embeddings)
...
def multi_gpu_batch_encode(model_loader_func, texts: List[str], batch_size: int, user_email: str = None) -> List[Embedding]:
    """
    ...
    embed_logger.info(f"Processing {len(batches)} batches across {len(gpu_devices)} GPU(s)")
        ...
        with ThreadPoolExecutor(max_workers=len(gpu_devices)) as executor:
            ...
            for batch_idx, batch in enumerate(batches):
                ...
                future = executor.submit(
                    ...
                    model_loader_func, batch, device, batch_idx
                )
            ...
            embed_logger.info(f"Completed: {successful_batches} batches, Failed: {failed_batches} batches")
    else:
        ...
        if batch_embeddings is not None:
            ...
            if isinstance(batch_embeddings, list):
                ...
                if torch.is_tensor(batch_embeddings):
                    ...
                    batch_embeddings_cpu = batch_embeddings.tolist() if hasattr(batch_embeddings, 'tolist') else list(batch_embeddings)
