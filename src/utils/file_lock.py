...
def save_jobs_safe(jobs_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        print(f"[FILE_LOCK] Successfully updated jobs.json")
        return True
    except Exception as e:
        ...
        with file_lock(jobs_file) as fd:
            ...
            if user_email in jobs:
                if isinstance(jobs[user_email], list) and len(jobs[user_email]) >= 2:
                    ...
                    print(f"[FILE_LOCK] Incremented job progress for {user_email}: {jobs[user_email][1]}/{jobs[user_email][0]}")
                    return True
                else:
...
def initialize_job_tracking_safe(user_email: str, total_jobs: int) -> bool:
    ...
    try:
        with file_lock(jobs_file) as fd:
            ...
            if user_email in jobs and isinstance(jobs[user_email], list) and len(jobs[user_email]) >= 2:
                ...
                print(f"[FILE_LOCK] Updated job tracking for {user_email}: {existing_completed}/{new_total} (added {total_jobs} new jobs)")
            else:
                # New user - start fresh
                jobs[user_email] = [total_jobs, 0]  # [total_jobs, completed_jobs]
                print(f"[FILE_LOCK] Initialized job tracking for {user_email}: 0/{total_jobs}")
